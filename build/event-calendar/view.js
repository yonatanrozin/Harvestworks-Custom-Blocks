...
function getURLDateStr() {
  ...
  try {
    // Check if the date is valid integer
    parseInt(param);
  } catch (e) {
...
function setMonth(dayStr) {
  ...
  document.querySelector("#calendar_date h3").innerHTML = date.toLocaleString("default", {
    month: "short"
  ...
  fetch("/wp-json/wp/v2/month?date=" + dayStr).then(res => res.json()).then(events => renderEventDays(dayStr, events));
